...
////
* xref:build_wildfly_server_and_boms[Build the WildFly server and BOMs]:
...
* https://github.com/jboss-developer/jboss-developer-shared-resources/blob/master/guides/CONTRIBUTING.adoc#jboss_developer_contributing_guide[Contributing Guide]: This document contains information targeted for developers who want to contribute to WildFly developer projects.
...
* Regardless of your level of expertise, we suggest you start with the link:helloworld/README.html[`helloworld`] quickstart. It is the simplest example and is an easy way to prove the server is configured and running correctly.
